...
async function testTaxConfiguration() {
  ...
  console.log(
    `PRO_PRODUCT_Tax_ID: ${process.env.PRO_PRODUCT_Tax_ID || 'Not set'}`
  );
  console.log(
    `NTCLIPBOARD_PRODUCT_PRICE_ID: ${process.env.NTCLIPBOARD_PRODUCT_PRICE_ID || 'Not set'}`
  );
  console.log(
    `PRO_PRODUCT_PRICE_ID: ${process.env.PRO_PRODUCT_PRICE_ID || 'Not set'}\n`
  );
  ...
  console.log(
    `Tax Rate for Checkout: ${getTaxRateForCheckout() || 'None (using automatic tax)'}\n`
  );
  ...
  try {
    ...
    if (process.env.NTCLIPBOARD_PRODUCT_PRICE_ID) {
      const ntPrice = await stripeServer.prices.retrieve(
        process.env.NTCLIPBOARD_PRODUCT_PRICE_ID
      );
      console.log(
        `NTClipboard Price: ${ntPrice.unit_amount / 100} ${ntPrice.currency.toUpperCase()}`
      );
    }

    if (process.env.PRO_PRODUCT_PRICE_ID) {
      const proPrice = await stripeServer.prices.retrieve(
        process.env.PRO_PRODUCT_PRICE_ID
      );
      console.log(
        `Pro Product Price: ${proPrice.unit_amount / 100} ${proPrice.currency.toUpperCase()}`
      );
    }
    ...
    line_items: [
      {
        ...
        quantity: 1,
        tax_rates: getTaxRateForCheckout()
          ? [getTaxRateForCheckout()]
          : undefined
      }
    ],
  ...
  console.log(
    `- Manual Tax Rate: ${checkoutConfig.line_items[0].tax_rates ? '✅ Configured' : '❌ Not set (using automatic)'}`
  );
  console.log();
